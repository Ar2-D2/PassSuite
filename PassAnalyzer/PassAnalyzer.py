#!/usr/bin/env python
# PassAnalyzer - Password Analysis tool
#
...
class StatsGen:
    def __init__(self):
        ...
        self.input_pw_count = 0
        self.output_stats_filepath = None
        self.output_csv_filepath = None
    ...
    def generate_stats(self, filename):
        ...
        ckpt = round(self.input_pw_count/100)
        ...
        with open(filename, 'r') as f:
            ...
            for password in f:
                ...
                if self.total_counter % ckpt == 0:
                    print(ts.timestamp(), "Passwords processed: ", int(self.total_counter/ckpt), "%", sep='')
    ...
    def print_stats(self):
        ...
        print(ts.timestamp(), "--- STATSGEN FINISHED ---")
        print()
        print()
        print("--- STATSGEN RESULTS ---")
        print()
        ...
        print("\n[*] Length:")
        for (length,count) in sorted(iter(list(self.stats_length.items())), key=operator.itemgetter(1), reverse=True):
        ...
        print("\n[*] Character-set:")
        for (char,count) in sorted(iter(list(self.stats_charactersets.items())), key=operator.itemgetter(1), reverse=True):
            if self.hiderare and not count*100/self.filter_counter > 0: continue
        ...
        print("\n[*] Simple Masks:")
        for (simplemask,count) in sorted(iter(list(self.stats_simplemasks.items())), key=operator.itemgetter(1), reverse=True):
            if self.hiderare and not count*100/self.filter_counter > 0: continue
            print(("[+] %25s: %02d%% (%d)" % (simplemask, count*100/self.filter_counter, count)))

        print("\n[*] Advanced Masks:")
        for (advancedmask,count) in sorted(iter(list(self.stats_advancedmasks.items())), key=operator.itemgetter(1), reverse=True):
    ...
    def write_stats(self, output_stats_filepath):
        """ Write password statistics. """
        with open(output_stats_filepath , 'a', newline='\n') as file: 
        
            file.write("\n" + ts.timestamp() + " --- STATSGEN FINISHED ---\n\n")
            file.write("\n--- STATSGEN RESULTS ---\n")
            
            
            file.write(("\n[+] Analyzing %d%% (%d/%d) of passwords" % (self.filter_counter*100/self.total_counter, self.filter_counter, self.total_counter)))
    ...
    def write_csv(self, output_csv_filepath):
        """ Write password statistics. """
        with open(output_csv_filepath , 'a', newline='\n') as file: 

            file.write(("Password Count;%d" % self.total_counter))
            ...
            file.write("\n\nPassword complexity:")
            file.write(("\ndigit (min/max);%s;%s" % (self.mindigit, self.maxdigit)))
            file.write(("\nlower (min/max);%s;%s" % (self.minlower, self.maxlower)))
            file.write(("\nupper (min/max);%s;%s" % (self.minupper, self.maxupper)))
            file.write(("\nspecial (min/max);%s;%s" % (self.minspecial, self.maxspecial)))

            file.write("\n\nSimple Masks:")
            for (simplemask,count) in sorted(iter(list(self.stats_simplemasks.items())), key=operator.itemgetter(1), reverse=True):
                if self.hiderare and not count*100/self.filter_counter > 0: continue
                file.write(("\n%s;%d" % (simplemask, count)))

            file.write("\n\nAdvanced Masks:")
            for (advancedmask,count) in sorted(iter(list(self.stats_advancedmasks.items())), key=operator.itemgetter(1), reverse=True):
                if count*100/self.filter_counter > 0:
                    file.write(("\n%s;%d" % (advancedmask, count)))
...
if __name__ == "__main__":
    ...
    statsgen.input_pw_count = input_pw_count
    
    # Get and Create Output Password Attributes
    output_stats_filename = fs.create_pw_filename('Stats', input_pw_name, input_pw_minlength, input_pw_maxlength, input_pw_count)
    output_stats_filepath = os.path.join(input_pw_filedir, output_stats_filename + '.txt')
    output_csv_filepath = os.path.join(input_pw_filedir, output_stats_filename + '.csv')

    statsgen.output_stats_filepath = output_stats_filepath
    statsgen.output_csv_filepath = output_csv_filepath
    
    if Path(output_stats_filepath).exists():
            os.remove(output_stats_filepath)
    ...
    with open(output_stats_filepath , 'a', newline='\n') as file: 
        ...
        if not options.quiet:
            file.write(header)
            print(header)
        file.write("\n" + ts.timestamp() + " --- STATSGEN STARTED ---\n")
        file.write("[*] Analyzing passwords in [%s]" % args[0])
        print(ts.timestamp(), "--- STATSGEN STARTED ---")
    ...
    statsgen.generate_stats(input_pw_filepath)
    statsgen.write_stats(output_stats_filepath)
    statsgen.write_csv(output_csv_filepath)
